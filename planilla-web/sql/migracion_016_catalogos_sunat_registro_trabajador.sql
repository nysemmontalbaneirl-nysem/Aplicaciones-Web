-- =========================================================================
-- Migracion 016: catalogos oficiales SUNAT (Anexo 2 T-Registro) + campos
-- que exige el T-Registro que faltaban en el alta de trabajador.
--
-- Contexto: el usuario pidio que cada campo de texto libre que tenga una
-- tabla oficial se convierta en desplegable (evita errores de digitacion),
-- y que se agreguen los datos que exige SUNAT segun una Constancia de Alta
-- del Trabajador real (Formulario 1604-1) que compartio como ejemplo. La
-- fuente de los catalogos es el Anexo 2 completo que subio
-- (docs/anexo2_tablas_parametricas_sunat.xlsx, 37 tablas T1-T37).
--
-- No destructiva: no se borra ni renombra ninguna columna existente.
-- entidad_bancaria, grado_instruccion y ubigeo (texto libre) se mantienen
-- tal cual estan - se agregan columnas *_codigo en paralelo para que el
-- formulario pueda migrar a desplegable sin perder los datos ya cargados.
-- Todo lo nuevo es opcional (NULL) salvo donde se indica un default seguro.
--
-- Decisiones confirmadas con el usuario (30/08/2026):
-- 1. Categoria ocupacional oficial SUNAT (T24) y tipo/regimen de
--    trabajador (T8/T33): se piden como campos independientes en el
--    formulario, no se derivan automaticamente de categoria_ocupacional.
-- 2. Establecimientos: JHCR declara varias obras como establecimientos
--    distintos, un proyecto = un establecimiento -> se agrega directo a
--    la tabla "proyectos" (no hace falta una tabla aparte).
-- 3. Regimen de salud: hay casos con EPS -> se agrega catalogo_eps.
-- 4. Ubigeo: se arma el catalogo completo (departamento/provincia/
--    distrito). Se agrega tambien "segunda_direccion" porque el usuario
--    senalo que la direccion del DNI muchas veces difiere de la
--    residencia real.
-- =========================================================================

-- -------------------------------------------------------------------------
-- Catalogos (todas son tablas de solo lectura para el sistema: se
-- actualizan corriendo esta migracion de nuevo con datos mas recientes de
-- SUNAT, no desde la aplicacion).
-- -------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS catalogo_tipo_documento (
...
CREATE TABLE IF NOT EXISTS catalogo_tipo_trabajador (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_grado_instruccion (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_regimen_pensionario (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_tipo_contrato (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_periodicidad (
    codigo VARCHAR(1) PRIMARY KEY,
    nombre VARCHAR(60) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_eps (
    codigo VARCHAR(11) PRIMARY KEY, -- RUC de la EPS ("0" = servicios propios)
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_tipo_pago (
    codigo VARCHAR(1) PRIMARY KEY,
    nombre VARCHAR(60) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_motivo_baja (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(200) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_categoria_ocupacional_sunat (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_regimen_salud (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_regimen_laboral (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_situacion_especial (
    codigo VARCHAR(1) PRIMARY KEY,
    nombre VARCHAR(150) NOT NULL
);
...
-- Ubigeo: la hoja fuente (T28) NO es una tabla jerarquica fila-a-fila, son
-- 3 listas independientes acomodadas en paralelo por maquetado. La
-- jerarquia real se deriva del propio codigo (los primeros 2 digitos del
-- codigo de provincia son su departamento; los primeros 4 digitos del
-- codigo de distrito son su provincia) - verificado contra la direccion
-- real de la constancia de ejemplo (PIURA-HUANCABAMBA-SAN MIGUEL DE EL
-- FAIQUE = departamento 19, provincia 1903, distrito 190307).
CREATE TABLE IF NOT EXISTS catalogo_ubigeo_departamento (
    codigo VARCHAR(2) PRIMARY KEY,
    nombre VARCHAR(100) NOT NULL
);

CREATE TABLE IF NOT EXISTS catalogo_ubigeo_provincia (
    codigo VARCHAR(4) PRIMARY KEY,
    nombre VARCHAR(100) NOT NULL,
    departamento_codigo VARCHAR(2) NOT NULL REFERENCES catalogo_ubigeo_departamento(codigo)
);

CREATE TABLE IF NOT EXISTS catalogo_ubigeo_distrito (
...
-- mas nueva del Anexo 2 en el futuro.
-- =========================================================================
